...
| Key | Action |
| --- | --- |
| `W` / `S` | Move forward / backward in the drone body frame |
| `A` / `D` | Strafe left / right in the drone body frame |
...
The script creates two `MultirotorClient` instances:
...
The split matters because `simGetImages` is a blocking RPC call. Instead of letting camera acquisition interrupt the flight loop, `drone_game.py` runs flight control in a background thread at `COMMAND_HZ` while the main thread fetches camera frames at `STREAM_FPS` and renders them with OpenCV.
